...
def load_jpg_ucf101(l, g, c, n, inference_class_names, transform):
    ...
    dir = os.path.join(
        "/home/kazuoki/AOSA2/data/ucf101/jpg/" + name + "/v_{}_g{}_c{}".format(name, str(g).zfill(2), str(c).zfill(2))
    )
...
def load_jpg_kinetics700(l, v, n, inference_class_names, transform):
    name = inference_class_names[l]
    class_dir = os.path.join("data/kinetics-700/val_jpg", name)
...
def load_jpg_my_data(l, g, c, n, inference_class_names, transform):
    name = inference_class_names[l]
    dir = os.path.join(
        "/home/kazuoki/AOSA2/data/mydata/jpg/" + name + "/v_{}_g{}_c{}".format(name, str(g).zfill(2), str(c).zfill(2))
    )
    path = sorted(glob.glob(dir + "/*"), key=numericalSort)
